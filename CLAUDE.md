...
M0–M4.5 complete. `frontend/` builds, 73 tests pass. `backend/` builds via `sam build`. The app has a working sidebar `GeneratePanel` (all sliders, seed + randomize, Generate button, JSON download/import) driving `MapCanvas`. Not yet visually verified in a browser by Claude (no browser-automation tool available in this environment) — dev server confirmed to serve without errors after every change so far, but the interactive parts need a manual check before treating any milestone as fully signed off.

**M4.5 (taxonomy migration) is done** — `NodeType` is now `settlement | wilderness | poi`; the old `mountain` type is a `mountain_range` `BoundaryMarker`; `ALGORITHM_VERSION` is `2.0.0`. No new generation *variety* yet (that's M4.6 — water/biome/civilian-scale/outpost-kind/poi-kind placement, `coastal`, `sea_route`, optional terrain zones). See "Notes on M4.5" below for implementation decisions made along the way. Next: M4.6, then M5 (edit panels).
