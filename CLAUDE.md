...
M0–M4 complete. `frontend/` builds, 72 tests pass. `backend/` builds via `sam build`. The app has a working sidebar `GeneratePanel` (all sliders, seed + randomize, Generate button, JSON download/import) driving `MapCanvas`. Not yet visually verified in a browser by Claude (no browser-automation tool available in this environment) — dev server was run and confirmed to serve without errors, but the interactive parts (pan/zoom/hover from M3, sliders/generate/import from M4) need a manual check before treating M3/M4 as fully signed off.

**Before M5, two new sub-milestones were inserted** — M4.5 (taxonomy migration) and M4.6 (generator capability expansion) — see spec Section 16 and Section 3c. These formalize a correction to the original spec: it had wrongly declared the generator could never touch `subtype`/water/extensions at all; the corrected rule is that the generator is capable through Tier 2 (`NodeType` → fork → `NodeSubtype`) plus `BoundaryMarker`, and only Tier 3+ detail plus `Faction` data stay permanently manual. **Not yet implemented — spec-only so far.** Next: M4.5 (types rewrite + mechanical migration, must land first and pass its own acceptance criteria), then M4.6 (new generation variety: water/biome/settlement-fork/terrain-zone placement), then M5 (edit panels).

## Notes on the Taxonomy Rework (M4.5/M4.6, pre-implementation)
