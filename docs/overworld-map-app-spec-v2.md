...
**M4.16.3 (proposed, not built) — human-led word-list and pattern curation pass.** M4.16.1/M4.16.2 both fixed *specific, named* problems, but the user's underlying request is broader: a session where the user reviews every pattern and every word list directly (most conveniently via `/nameGenerator`'s existing "Word lists" review section and per-pattern batch generation) and curates entries back down to a consistently good bar, rather than Claude guessing at what else might be off-tone. Candidates already flagged during M4.16.2 but deliberately not acted on unilaterally: `settlementMedieval`'s `"settlement type"` suffix's `"Don"`/`"By"`/`"Thwaite"`/`"Garth"`; whether `FAUNA`'s smaller/more obscure entries (`"Vole"`, `"Marten"`, `"Ferret"`, `"Kestrel"`, `"Adder"`, `"Grouse"`) belong; whether every race theme's 17 root/suffix categories (capped at 16-18 per M4.16.1's own notes) hold up on a full read-through. Scope still to be defined with the user when this milestone starts — likely a mix of direct list edits and possibly a lightweight "flag this word" affordance on the `/nameGenerator` page if reviewing via raw file edits proves tedious.
