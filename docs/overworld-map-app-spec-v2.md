...
## 7f. Layout relaxation (`src/core/layoutRelax.ts`) — M4.10
...
### M4.9 — Fine-tuning params (hardcoded constants made adjustable)

Deliverables: eleven behaviors that were hardcoded constants become `GenerationParams` fields, each defaulting to exactly the value it was hardcoded to. Six are radial-specific (Section 7e): `radialInwardWeight` (was `INWARD_WEIGHT_FLOOR`), `radialFalloffExponent` (the interconnectivity falloff curve, previously fixed linear), `radialJitter`, `radialRimFraction` (the "mid" band's inner edge scales with it via `MID_TO_RIM_RATIO`), `radialClusterMaxSize`, `radialClusterSpread`. Five apply under **both** placement algorithms since they live in `core/generationShared.ts`'s geometry-agnostic taxonomy logic: `maxLargeSettlements` (was `MAX_CITY_OR_ABOVE`), `roadFraction` (was a fixed 50/50 roll), `coastalChance` (was `INDEPENDENT_COASTAL_CHANCE`), `interiorBoundaryDamping` (was `MID_ZONE_BOUNDARY_DAMPING`), `wildernessCheckMultiplier` (was a fixed ×0.2 on `checkRequiredFraction`). `core/shareCode.ts`'s `PARAMS_CODEC_VERSION` → `5` (`encodeV5`/`decodeV5`; `decodeV1`–`decodeV4` backfill the previously-hardcoded values). `GeneratePanel.tsx` gains a collapsible "Advanced tuning" section (the six radial-only sliders shown only when Radial is selected).

Acceptance: the full test suite — including every determinism/"identical structural output" test for both placement algorithms — passes unchanged after the extraction, proving no default actually changed behavior; `ALGORITHM_VERSION` does **not** bump (no RNG call sequence changed); each knob has a test demonstrating a measurable effect (e.g. `radialJitter` via lattice-position deviation, `radialRimFraction` via boundary-marker count, `maxLargeSettlements` via a hard cap under both algorithms) rather than just absence of crashes; every knob stays valid (clean `validateMap`) at both extremes; an old (pre-M4.9) share code still decodes and regenerates its original map exactly.

### M4.10 — Layout relaxation pass

Deliverables: `core/layoutRelax.ts` (new) — a geometry-only visual pass (Section 7f) applied to the output of **both** placement algorithms inside `generateMap`, before `markCheckRequired`/`generateTerrainZonesStep`. A Magnetic Spring Model relaxation (Sugiyama & Misue, 1995): a magnetic force rotates each edge toward the `CompassDir` it already declares (never relabeling it), and a separation force — applied as a **constraint projection after** the magnetic step, not as a competing force — normalizes node spacing. `ALGORITHM_VERSION` → `2.4.0` (existing seeds render with different, improved positions). Three new params: `layoutRelaxStrength` (0 disables the pass entirely, restoring exact pre-M4.10 placement), `layoutNodeSpacing`, `layoutDirectionWeight`. `core/shareCode.ts`'s `PARAMS_CODEC_VERSION` → `6` (`decodeV1`–`decodeV5` backfill the new defaults, not a behavior-preserving "off," since the version bump already breaks byte-identical reproduction for old links). `GeneratePanel.tsx` gains a "Layout polish" group in the main panel (`layoutRelaxStrength`) plus the other two knobs in Advanced tuning.
...
### M4.12 — Generated names (nameforge library + naming pass)

Deliverables: `frontend/src/nameforge/` (new) — a standalone, dependency-free random-name-generation library (word-bank "slots" assembled via "patterns," grouped into "themes"), with zero imports from anywhere outside itself so it can be extracted and published on npm later by copying the folder and adding a `package.json` (see its own `README.md`). Themes: `settlementMedieval`, `poiFanciful`, one per `Biome` (`wildernessForest`/`wildernessSwamp`/`wildernessDesert`/`wildernessTundra`/`wildernessJungle`/`wildernessPlains`), `waterFeature`, `regionName` (the map's own name), and `elvishSyllable` (a demonstration of the library's `syllableChain` slot type for invented-language names — not wired into any node/map naming yet, since nothing in the current taxonomy calls for a culture/language concept). `core/naming.ts` (new) — the only file bridging the generic library to this app's types: `applyNodeNames(nodes, rng)` picks a theme per node from its `type`/`subtype`/water-branch fork and overwrites `label`; `generateMapName(rng)` produces the map's `name`. Both are a **separate post-generation pass**, called from `generateMap()` at the same pipeline seam `layoutRelax.ts` (M4.10) already runs at — mirroring that pass's shape exactly (a standalone function over already-classified nodes, no geometry/edge awareness) rather than being baked into node classification or built as a formal generic "step pipeline" (considered and explicitly rejected as more infrastructure than naming alone needs). `core/generator.ts`'s `ALGORITHM_VERSION` → `2.5.0` (a genuine new capability consuming `rng()` draws that didn't exist before). The old hand-curated, never-wired-in `core/names.ts` is deleted — nameforge is the "future randomize-name feature" its own header comment was reserving room for. `store/mapStore.ts` gains `setMapName`/`rerollMapName` (metadata edits, not graph edits — no undo/redo history push, matching `updateDraftParam`/`randomizeSeed`). `GeneratePanel.tsx` gains a "Map name" field + 🎲 reroll button at the top of the sidebar.

Acceptance: `nameforge`'s own test suite (pure, no DOM) proves determinism (same rng sequence → same name), that `rerollSlot` changes only the targeted slot, that `regenerate` keeps the same pattern while changing its words, and that every theme/pattern produces non-empty text; `core/naming.ts`'s tests prove every `NodeType`/biome/water-branch combination gets a real (non `"Village-1"`-style) label, deterministically by seed; every pre-existing determinism test in `generator.test.ts`/`radialGenerator.test.ts` (same-seed-twice structural comparison, not a hardcoded golden string) passes unchanged; visually verified via headless-Chrome render of a freshly generated map — names read as thematically appropriate (medieval settlements, biome-flavored wilderness, dark/fanciful POIs, possessive/compound water features) with no placeholder-style labels remaining. No `PARAMS_CODEC_VERSION` change — share codes only encode `GenerationParams`, and naming is unconditional (no new tunable/opt-out field), matching the project's existing "no speculative mode field" preference.

### M4.13 — Name generator test page (`/nameGenerator`)

Deliverables: the app's first client-side route beyond the single root page — `react-router-dom` added as a real dependency (not a hand-rolled path check; routing is now a genuine recurring need), `main.tsx` wraps `<App/>` in `<BrowserRouter>`, `App.tsx` becomes route registration (`/` → new `pages/MapApp.tsx`, the unchanged former `App.tsx` body; `/nameGenerator` → new `pages/NameGeneratorPage.tsx`). `frontend/public/_redirects` (Netlify SPA fallback: `/* /index.html 200`) added proactively — without it, a direct visit or refresh on any client-side route other than `/` 404s on Netlify's static hosting. Two small, targeted `nameforge` API additions the tool genuinely needs: `generateFromPattern(theme, patternId, rng?)` (force a specific pattern instead of a random one) and `nameforge/inspect.ts`'s `describePattern`/`describeSlot` (plain-text slot-sequence summaries for an "internals" panel), plus a new `ALL_THEMES` export so tooling can enumerate every registered theme. New `pages/nameGeneratorBatch.ts` — pure, unit-testable batch-generation logic (deliberately extracted out of the React page component) — `generateBatch(theme, patternId, baseSeed, count)` assigns each result in a batch its **own** seed (`baseSeed + index`, not one shared RNG stream), and `reproduceResult(theme, result)` is the one place that knows how to turn a result's recorded `{patternId, patternForced, seed}` back into the identical `GeneratedName`. The page itself: theme/pattern/seed/count controls, a results table with per-slot reroll (`rerollSlot`, unseeded/live), an "edited" flag once any row is touched, a JSON view + "Copy JSON" button, and an "Internals" panel listing the selected theme's patterns via `describePattern`.

Acceptance: `generateBatch`/`reproduceResult` are unit-tested — full-batch regeneration is byte-identical from the same inputs, and (the specific regression this caught) **any single result is independently reproducible from only its own recorded metadata**, in both "random pattern" and "forced pattern" modes. This surfaced a real bug during implementation: `generateName`'s first `rng()` draw picks the pattern before any slot content is drawn, so naively reproducing a "random pattern" result later via `generateFromPattern(patternId, makeRng(seed))` — skipping that draw — silently produced different slot content even though the recorded `patternId` still matched. Fixed by recording a `patternForced` flag per result and having `reproduceResult` dispatch through the same function (`generateName` vs. `generateFromPattern`) the result was originally produced by, not just replaying `patternId`. Visually verified via headless-Chrome screenshot of both routes (root unchanged, `/nameGenerator` renders its controls correctly) plus direct script-level verification of the batch/reproduce logic. Not verified by Claude: the actual click-through of "Generate batch"/per-slot-reroll buttons in a live browser — no browser-automation tool available, same accepted limitation as every other interactive control in this app. Reachable in production (not dev-gated) by explicit user choice — an informed exception to the project's "hide the algorithm's internals" precedent, scoped to names only, not the core placement/generation algorithm.

### M4.13.1 — Word list review section (`/nameGenerator`)

Deliverables: `nameforge/types.ts`'s bank `Slot` variant gains an optional, purely descriptive `name` field (e.g. `"roots"`, `"suffixes"`) — generation itself never reads it. Every theme file tags its bank slots with a category name (`themes/_shared.ts`'s `rootSuffixPatterns` helper, `themes/settlement.ts`, `themes/poi.ts`, `themes/region.ts`, and the bespoke `waterFeature` patterns in `themes/wilderness.ts`). New `nameforge/inspect.ts`'s `listWordLists(theme): NamedWordList[]` walks every pattern's slots and returns one entry per distinct **named** bank (deduped by name, not array identity — several themes derive a second, differently-cased copy of the same conceptual list for a different pattern, e.g. a capitalized two-word form vs. a lowercased compound form; those are one category to a human reviewer, not two), plus a `syllableChain` slot's `start`/`middle`/`end` pools each as their own named entry. `NameGeneratorPage.tsx` gains a "Word lists" section (open by default, unlike the existing "Internals — patterns" section) showing every category's word count and full contents for the currently selected theme.

Acceptance: `listWordLists` is unit-tested per theme — correct name/count for `settlementMedieval`'s two banks, dedup verified explicitly on `poiFanciful` (whose compound-form noun bank is a separately-derived lowercase copy of its two-word-form noun bank), the water feature theme's two distinct root banks (`"roots"` vs. `"possessive roots"`) staying distinct, and `elvishSyllable`'s syllable pools surfacing as three separate named lists; every registered theme produces at least one non-empty list. Visually verified via headless-Chrome screenshot of the rendered section, and a direct script run printing every theme's word lists to confirm content/dedup by eye.

### M4.14 — Categorized word banks + 3 distinct region-name patterns

Deliverables: `nameforge/types.ts`'s `Bank` becomes `readonly string[] | { categories: readonly WordCategory[] }` — a categorized bank picks a category **uniformly first**, then a word **uniformly within it** (`generate.ts`'s `pickFromBank`, two `rng()` draws instead of one), so a category's word count no longer determines its influence on output (a 2-word category and a 7-word category each land close to `1/categoryCount` of picks). Every theme's roots/suffixes (or adjectives/nouns) banks are recategorized into 2-7 sensible sub-categories apiece (colors, gems, materials, flora, fauna, landscape, directions, narrative descriptors, etc., tailored per theme) — same words as before, reorganized, not expanded. `inspect.ts`'s `listWordLists` now emits both the flattened aggregate (under the bank's existing name, e.g. `"roots"`) and one entry per category (`parent` set to the aggregate's name), so the `/nameGenerator` review UI can show category sub-tables nested under their aggregate table. `regionName` gained two genuinely new pattern shapes alongside its existing descriptive phrase — a compact single-word compound (`"Amberhold"`) and a nested genitive (`"The Moors of Dark Ravenholt"`: noun + "of" + a narrative-descriptor + an embedded compound name) — for real structural cadence variety in map names, not just word-choice variance. `_shared.ts`'s `rootSuffixPatterns`/`rootSuffixTheme` gained an optional `labels` parameter so a theme whose two halves aren't literally "roots"/"suffixes" (e.g. `poiFanciful`'s adjectives/nouns) can label them accurately in the review UI. `core/generator.ts`'s `ALGORITHM_VERSION` → `2.6.0`.

Acceptance: a statistical test confirms category-weighted picking holds — across 5000 draws from a bank with categories ranging 2–7 words, every category's observed share lands within a wide but meaningful tolerance of `1/categoryCount`, and a 2-word category's pick count is **not** swamped by a 7-word category's (far from the ~3.5x a size-proportional pick would produce). `listWordLists` tests cover the aggregate-plus-categories shape and confirm dedup still holds when a category is referenced both inside an aggregate and directly by another slot (`regionName`'s nested pattern reuses its narrative-descriptors category standalone). `regionName`-specific tests assert each of the 3 patterns produces its intended shape (no spaces in `compound`; no nested `"of"` in `the-root-noun`; both a `"The X of Y Z"` shape and an unspaced embedded compound in `nested-of`) and that the same seed produces 3 visibly different strings across the 3 patterns. Full existing suite passes unchanged in shape (determinism, non-empty output) since only content/behavior changed, not any public API. Verified visually via headless-Chrome screenshot of the `/nameGenerator` "Word lists" section showing category sub-tables correctly nested under their aggregate. This milestone was preceded by real back-and-forth with the user correcting a category/pattern conflation (a direction word and a color word filling the same slot in "The ___ Noun" is one pattern, not two) — see `CLAUDE.md`'s "Notes on M4.14."

### M4.15 — Shared category library, deeper word lists, ≥3 patterns per theme, pattern rarity

Deliverables: `nameforge/categories/` (new) — generic `WordCategory` constants (`COLORS`, `GEMS`, `MATERIALS`, `FLORA`, `FAUNA`, `DIRECTIONS`, `LANDSCAPE_DESCRIPTORS`, `NARRATIVE_DESCRIPTORS`, `PERSONAL_NAMES`), each 15-16 words, reused by reference across themes instead of hand-duplicated per theme; every remaining theme-specific category expanded to 8-12 words. Sharing the same category *object* across themes is deliberate: it keeps near-identical lists from drifting apart, and makes the occasional cross-theme flavor collision (a desert region drawing a "verdant" color) emerge from shared data rather than copy-paste divergence. `nameforge/types.ts`'s `Pattern` gains an optional `weight` (default 1); `generate.ts`'s `pickPattern` selects weight-proportionally rather than uniformly, so a theme can mark a pattern deliberately rare without a separate probability system. Every theme now has ≥3 patterns: `settlementMedieval` (was 1) gains `possessive` ("Devon's Ford" — a personal name possessing a place, the real-world-regional flavor) and `descriptive` ("Northern Bridge" — two standalone words, not glued); `waterFeature` (was 2) gains `descriptive` ("The Silver Pool"). `core/generator.ts`'s `ALGORITHM_VERSION` → `2.7.0`.

Acceptance: weighted pattern selection is unit-tested against an isolated test theme — a `weight: 0.2` pattern alongside a `weight: 1` one lands near its 1/6 expected share across 5000 draws, not the 50/50 a uniform pick gives, and patterns with no weight set still behave exactly uniformly (regression). `listWordLists` tests confirm a shared category reports identical contents wherever it's reused (`COLORS` in both `settlementMedieval` and `regionName`), that every category in every theme now has ≥5 words, and that every theme except the `elvishSyllable` demonstration has ≥3 patterns. Verified visually via headless-Chrome screenshot of `/nameGenerator` (settlement's roots aggregate now 101 words across 7 categories, suffixes 37 across 4) and by direct script sampling of each theme's patterns.

**Roadmap — this is the first of four planned naming milestones** (the user's full request spans all four; they're sequenced rather than shipped as one unreviewable change):
- **M4.16** — 17 race/species naming themes (dwarvish, orcish, halfling, gnomish, aasimar, dragonborn, goliath, tiefling, drow, minotaur, illithid, beholder, goblin, lizardfolk, giant; elf exists, human is covered by M4.15's regional settlement flavor), each with its own word lists and ≥3 patterns — syllable-chain-based for the alien-sounding races, root+suffix-based for the language-like ones.
- **M4.17** — `generateFromThemeGroup` (a generic weighted multi-theme picker: "decide on a style, then generate in it") plus wiring it into `core/naming.ts`/`generateMap()` via a new `settlementCultureMix` `GenerationParams` field shaped exactly like the existing `biomeMix` (per-node weighted pick, no new spatial-clustering machinery). Real `PARAMS_CODEC_VERSION` + `ALGORITHM_VERSION` surface area and a `GeneratePanel.tsx` slider group mirroring the biome-mix one.
- **M4.18** — POI overhaul: far wider iconic-landmark noun/adjective variety (`The Crystal Spire`, `The Black Lighthouse`, `Demon's Horn`, `Red Chasm`), plus a "legendary figure" pattern (`The Blackened Fields of the Archlich Vartailian`) built on a new `legendaryFigure` theme (title/epithet + invented surname) and a new `Slot` variant that embeds a nested `generateName` call — reusable well beyond that one pattern, and marked low-`weight` so legendary figures stay rare.

### M4.16 — 16 race/species naming themes

Deliverables: `nameforge/themes/races/` (new) — one theme per D&D species (`dwarvish`, `elvish` — rebuilt from M4.12's single-pattern `elvishSyllable` demonstration, now retired — `halfling`, `gnomish`, `aasimar`, `draconic`, `goliath`, `tiefling`, `drow`, `orcish`, `goblin`, `minotaur`, `lizardfolk`, `giant`, `illithid`, `beholder`; human isn't a separate theme — settlementMedieval's M4.15 possessive/descriptive patterns already cover human-flavored place names). `_shared.ts`'s `raceTheme(spec)` gives every race the same three-cadence template — `native` (pure syllable-chain invented word), `compound` (common-tongue root+suffix), `native-of` ("The [suffix] of [native name]") — so cross-race variety comes from phonetics and word content, not structure; a race can add culture-specific extra patterns (`goliath`'s hyphenated deed-names via `compoundSeparator: "-"`, `tiefling`'s virtue-name pattern). Two real engine bugs surfaced and were fixed during content authoring, both now permanently regression-tested: `generate.ts`'s `renderSyllableChain` collapses any run of 3+ identical letters from a syllable join (`"Dill"+"ly"` → `"Dillly"` → `"Dilly"`) and now avoids repeating the immediately-previous middle/end syllable (`"Ghyl"+"uun"+"uun"` → stutters); every theme's root bank was audited against its suffix/noun bank for shared words or word-stems (`"Barren"`/`"barrens"` → `"The Barren Barrens"`), which caught issues in several M4.15 themes too (`settlementMedieval`, `wildernessSwamp`, `wildernessJungle`, `regionName`), not just the new race ones.

Acceptance: every race theme is unit-tested for the three-cadence template, non-empty output from every pattern, per-seed determinism, zero root/suffix word-or-stem collisions (checked across *all* themes, not just races), zero triple-letter runs across 300 samples per race, and that a native name still starts/ends with one of its own declared boundary syllables after the letter-run fix. `core/naming.ts`/`generateMap()` are untouched — no race theme is wired into automatic map generation yet (that's M4.17) — confirmed by inspecting `core/naming.ts`'s imports and generating a sample map to confirm identical structure/`ALGORITHM_VERSION` (no bump this milestone). Verified visually via headless-Chrome screenshot of `/nameGenerator` and direct script sampling of all 16 races across all patterns.

### M4.16.1 — Deeper word lists, two new settlement patterns, race epithet variant

Deliverables: a broad content-depth pass across every already-shipped nameforge theme, prompted directly by user feedback on repetitiveness. `categories/generic.ts`: `PERSONAL_NAMES` grew from ~12 to 50 words (the most-reused category, so the one most likely to repeat); `FAUNA` grew to 30; a new `FANTASY_CREATURES` category (26 words — dragon, griffon, minotaur, harpy, hydra, etc., Greek-myth/core-D&D flavor) and a new `EPITHETS` category (24 words — Bold, Wise, Cruel, Swift, etc.) were added; every other generic category grew to 22-24. `settlementMedieval` gained two new patterns — `beast-descriptive` ("Red Dragon Hall") and `beast-possessive` ("Crystal Griffon's Barrow") — an adjective (color/gem/material/landscape-descriptor) + a flora/fauna/fantasy-creature subject + a suffix, giving settlement 5 patterns total; its `structures` category grew to 20 and its four suffix categories were each roughly doubled (16-20 words). Every race theme (`_shared.ts`'s `raceTheme`) gained a fourth base pattern, `native-of-epithet` ("The Crown of Daniel the Bold" — the existing `native-of` shape plus a trailing epithet), and every one of the 17 race themes' root/suffix categories grew from their original M4.16 8-10 words to 16-18. `wilderness.ts`'s six biome themes and `waterFeature` had every category pushed to 16-20 (from M4.15's 8-16), and `poi.ts`'s ominous/burial/structures/landscape categories grew to 20 each. `core/generator.ts`'s `ALGORITHM_VERSION` → `2.8.0` (bigger banks and settlement's two new patterns both shift the naming pass's `rng()`-to-output mapping for every seed; race themes still aren't wired into `generateMap()`, so their own changes don't factor into the bump — that's still M4.17).

The pattern-scoped collision test (`tests/nameforge/races.test.ts`) was redesigned during this pass, not just re-run: the original version compared a theme's `roots` bank against *every other named bank in the theme*, which produced false positives once themes had multiple independent root/suffix bank pairs that intentionally share source categories but never appear together in one pattern (e.g. settlement's plain `roots`/`suffixes` vs. its new `beast-adjectives`/`beast-subjects`/`suffixes`). The corrected version only compares bank slots that are actual siblings within one `Pattern`. Re-run against the *existing* (pre-this-pass) content, it caught two real, previously-shipped M4.14 bugs in `regionName`'s `nested-of` pattern (`NOUNS`'s `"Vale"`/`"Moor"`/`"Reaches"` vs `COMPOUND_SUFFIXES`'s `"vale"`/`"moor"`/`"reach"`, and a leading narrative-descriptor slot that could coincidentally match the identical word embedded in its own compound-root slot) — both fixed as part of this milestone.

Per explicit user instruction, categories were expanded toward a 20-25-word target only as far as quality allowed, with any exception documented rather than padded: `waterFeature`'s `WATER_POSSESSIVE_ROOTS` "other" category stays at 7 words (a broad not-an-occupation catch-all doesn't have as large a natural vocabulary in the "old miller/fisher/widow" tone without reaching), and the 17 race themes' root/suffix categories were capped at 16-18 rather than pushed to 20-25 — several (illithid's "the mind"/"the colony", beholder's "paranoia", minotaur's "the labyrinth") are narrow enough in concept that words past that point started reading as synonym padding or off-tone.

Acceptance: full existing suite passes (219 tests) plus the redesigned collision test; every content change was verified via the pattern-scoped collision test, iterating on individual word substitutions until clean (documented inline at each fix site, e.g. `settlementMedieval`'s locally-filtered `BEAST_COLORS` excluding `"Ashen"` to avoid `"Ashen Ash Hall"`). `npm run build` and `npm test -- --run` both green.

**Proposed future longer patterns (not built — for a future sub-milestone):**
- **Settlement "dual-feature" toponym** (`settlementMedieval`): a hyphenated compound-plus-preposition-plus-feature form mirroring real English toponyms like "Stratford-upon-Avon" or "Newcastle-under-Lyme" — e.g. `"Ashford-upon-Mere"`, `"Millbrook-under-Fell"`. Reuses existing root/suffix/landform categories with no new content needed, just a new pattern shape (`[compound]-upon-[water or landform feature]`).
- **RegionName "founding dynasty"** (`regionName`): a longer nested form naming a region after a founding house/lineage — e.g. `"The Everlasting Realm of House Ashgrove"` — combining a narrative-descriptor-flavored realm noun with "House" plus an invented surname-like compound (reusing the same compact-compound shape `regionName`'s other patterns already use). Distinct from the existing `nested-of` pattern's narrative-descriptor framing; gives regions a dynastic/political flavor `regionName` doesn't currently have.

### M4.16.2 — Word-list quality cleanup + compound-pattern fix

Deliverables: a targeted quality-fix pass, prompted directly by the user reading M4.16.1's output and flagging specific problems, distinct from that milestone's *depth* pass. `categories/generic.ts`: removed `FAUNA`'s `"Kite"`/`"Vixen"`/`"Ermine"`, `FLORA`'s `"Larch"`/`"Sorrel"`, and `MATERIALS`' `"Chalk"` — all flagged by the user as too obscure/off-tone. Also fixed a real bug found while addressing the same complaint: `LANDSCAPE_DESCRIPTORS`' `"Wind-carved"` contained a literal hyphen, which rendered as `"Wind-carvedford"` once glued into any compound pattern — replaced with `"Windworn"`. `settlementMedieval`'s `"settlement type"` suffix category dropped `"Ham"`/`"Toft"`/`"Stow"`/`"Ness"` (flagged as too obscure) in favor of plainly-readable `"Village"`/`"Town"`; `"Don"`/`"By"`/`"Thwaite"`/`"Garth"` are similarly obscure but weren't named explicitly, so were left for M4.16.3's broader review rather than removed unilaterally here. `settlementMedieval`'s `compound` pattern — reported as "only works about 10% of the time" — had its `ROOTS` bank narrowed to exclude `DIRECTIONS`: a multi-syllable modifier like `"Northeastern"`/`"Windward"` reads fine before a space-separated noun (the `descriptive` pattern already uses it that way) but not glued directly onto a suffix with no separator (`"Northeasternford"`), which was the single biggest source of the reported bad output. `core/generator.ts`'s `ALGORITHM_VERSION` → `2.8.1` (smaller/reshuffled banks shift which word a given `rng()` draw resolves to, a real output change for every seed even though nothing here is a new capability).

Acceptance: full existing suite passes (219 tests), `npm run build` and `npm test -- --run` both green. Verified via direct script sampling of the `compound` pattern specifically (40 samples across a spread of seeds) — the DIRECTIONS-exclusion fix visibly raised the proportion of names that read as plausible fantasy/real-world toponyms (`"Claystead"`, `"Eaglewood"`, `"Ceruleanhaven"`, `"Ashenbourne"`, `"Otterburg"`) rather than the previous awkward direction-glued forms.

### M4.16.3 — Human-led word-list curation pass (checklist review)

Deliverables: the broader review M4.16.2 deferred, done as an actual human-in-the-loop pass rather than more Claude-guessed fixes. Claude read every word list in the library (generic categories, settlement, region, all six wilderness biomes plus `waterFeature`, poi, and all 17 race themes) and wrote every plausibly-obscure/archaic-medieval/technical-jargon word to a checklist document (`docs/word-list-cleanup-candidates.md`, git-untracked — a working review artifact, not permanent project documentation), grouped by file/category with a one-line reason per word and a `[ ]`/`[x]` per entry (`[x]` = keep, unmarked = delete). The user marked the file directly and returned it; Claude then deleted every unmarked word from the actual theme files. Two items were flagged separately as **bugs, not taste calls** (found while scanning, not user-reported): halfling's `"Pantry Row"` and gnomish's `"Workshop Row"` suffix words both contained a literal embedded space, which rendered as a broken result once glued into the compound pattern (e.g. `"honeypantry row"`) — the same defect class as M4.16.2's `"Wind-carved"` hyphen bug. Categories were **not** backfilled to any target word count after deletion — a smaller, all-recognizable list was treated as strictly better than a padded one, which is the whole reason this pass exists. `core/generator.ts`'s `ALGORITHM_VERSION` → `2.8.2` (the deletions touch settlement/region/wilderness/poi, all wired into `generateMap()`, so this is a real output change for every existing seed; the race-theme deletions don't factor in, since race themes still aren't wired into map generation).

Acceptance: full existing suite passes (219 tests) with zero content-side test changes needed (pure removals can't introduce a new bank-slot collision), `npm run build` and `npm test -- --run` both green.

### M4.16.4 — Sample-output review: pattern-level fixes

Deliverables: the natural sequel to M4.16.3, but reviewing *generated output* instead of *word lists* — 10 real samples (via `generateFromPattern`, not hand-written) from every pattern of every theme, written to `docs/nameforge-sample-review.md` (git-untracked, same working-artifact convention as M4.16.3's checklist) with Claude's own pre-read flags plus a note that the checkbox convention is inverted from the word-list doc (`[x]` = "flag this," not "keep"). The user's feedback was almost entirely pattern-level rather than word-level, which drove two structural fixes:

1. **`regionName`'s `compound` pattern had the exact DIRECTIONS-gluing bug `settlementMedieval`'s `compound` had before M4.16.2** — caught in the reviewer's own sample (`"Northwesternrun"`). Fixed identically: new `COMPOUND_ROOTS`/`EMBEDDED_ROOTS` banks (excluding `DIRECTIONS`) feed the two patterns that glue with no separator (`compound`, `nested-of`'s embedded half); `ROOTS` itself is untouched since `the-root-noun` still uses directions correctly (space-separated).
2. **`_shared.ts`'s `rootSuffixPatterns` (used by `poiFanciful` + all six wilderness biomes) dropped from three patterns to one.** The reviewer questioned whether the glued `compound` pattern added value distinct from the two-word forms, and separately proposed merging `two-word`/`the-two-word` into one pattern with a coin-flip "The" prefix. Root cause identified for the first: settlement's suffixes are genuine bound English place-name suffixes (`-ford`, `-mere`), so gluing them reads like a real town name; poi's/wilderness's "suffixes" (`Gate`, `Rift`, `Meadow`, `Precipice`) are full standalone nouns, so gluing one onto an adjective just produces an unwieldy run-on word with no real naming convention behind it — confirmed by the sample review itself (`"Forsakenunderdepth"`). Settlement's own compound pattern is intentionally untouched (hand-written directly in `settlement.ts`, not built from this helper) since its suffix vocabulary genuinely is glue-shaped. The merge itself needed no new engine capability: the leading "The " became a `["", "The "]` 2-entry bank pick (reusing the existing uniform bank-choice machinery) instead of two separate `Pattern` entries — extending the M4.14 "a category is not a pattern" lesson to "a stylistic coin-flip prefix is not a pattern either." Combined with dropping `compound`, these 7 themes now have exactly 1 pattern each.
3. A handful of specific words were also removed per direct reviewer feedback: settlement's `"Worth"`/`"Chester"` (homograph risk / already reads as a real place name) and `"Dovecote"` (ambiguous once glued — replaced with `"Rectory"`), desert's `"Nothing"`, waterFeature's `"Eddy"` (blurred into `"Husheddy"`), swamp's `"Rank"` (read as a literal dirty sink).
4. **Deferred, not fixed**: the same "gluing a long root onto a full-noun suffix" issue turned up occasionally in dwarvish/beholder/lizardfolk/illithid's `compound` patterns (e.g. `"Underdarkworkshop"`, `"Overcautiousaerie"`). The user chose to leave these for a later pass — lower priority since race themes aren't wired into map generation yet (M4.17) and the occurrences are rarer (those themes mix genuine short suffixes with the occasional long full-noun one, unlike poi/wilderness where every suffix has the problem).

Two tests updated for the structural change: `races.test.ts`'s bank-slot collision check now skips empty-string bank entries (the `["", "The "]` prefix bank would otherwise false-positive against every sibling word, since `"".startsWith(anything)` is trivially true); `inspect.test.ts`'s pattern-count and per-list-depth rules now carve out the 7 one-pattern themes and the 2-entry "the (optional)" bank respectively. `core/generator.ts`'s `ALGORITHM_VERSION` → `2.9.0` (a minor bump, not patch — this changes pattern *structure*, not just word content, the same class of change M4.14/M4.15's pattern additions were).

Acceptance: full existing suite passes (219 tests) after the two test updates above; `npm run build` and `npm test -- --run` both green. Verified via direct script sampling before/after each fix (30 `regionName` compound samples with zero direction-glued results post-fix; 8 samples each from `poiFanciful`/`wildernessForest`/`wildernessDesert` confirming exactly one pattern remains and the "The" prefix mixes roughly 50/50).
